...
function available(callback) {
    if (!openconnectBinary) {
        ...
        kext.ensure(['foo.tun', 'net.tunnelblick.tun', 'com.viscosityvpn.Viscosity.tun'], '/Library/Extensions/tun.kext', function (err, loaded) {
            if (err) {
                return callback(err);
            }
            if (!loaded) {
...
function connect(config, extraOptions, callback) {
    ...
    exports.dlog('sudo', args);
    child = sudo(args);
    child.on('started', function () {
        exports.dlog('started');

        child.stdout.on('data', function (data) {
            data.toString().split('\n').forEach(function logStdout(line) { exports.dlog(line); });
        ...
        child.stderr.on('data', function (data) {
            data.toString().split('\n').forEach(function logStderr(line) { exports.dlog(line); });
...
function disconnect(callback) {
    exports.dlog('sudo -s kill ' + child.pid);
